...
import useDoubleClick from '@/hooks/useCustomDoubleClick';
// const pinSvg = '/fd_pin.svg'
import { PIN_ICON_DATA } from '@/components/PinIcon';
...
function CanvasComponent({pdfId, zoomLevel, onZoomChange}) {
  ...
  const currentPlan = plans.find((plan) => plan.id === pdfId);
  ...
  const renderPoints = useCallback(() => {
    ...
    points.forEach((point, index) => {
      // Draw the pin image (centered on point)
      // Draw the pin
      // context.fillStyle = elementConfig["pin"];
      // context.fillRect(point.x - 5, point.y - 5, 10, 10);
      const dimensionMultiplier = 30;
      const pinWidth = dimensionMultiplier * 800/1080; // Adjust size as needed
...
const findClosestPin = useCallback(
  ...
  const doubleClickEvents = useDoubleClick({
    onDoubleClick: handleDoublePointerDown,  // Use existing handler
